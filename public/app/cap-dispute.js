...
async function aiDisputeSummary(chitId, disputeId){
  ...
  try{
    ...
    var html='<style>'+(typeof _AI_MDCSS!=='undefined'?_AI_MDCSS:'')+'</style>'
      +'<div style="font-size:11px;color:#6d5bd0;background:#f2effc;border:1px solid #ddd4f5;border-radius:8px;padding:8px 11px;margin-bottom:12px;line-height:1.5">🤖 <b>AI summary — neutral, informational.</b> It does not decide the dispute. '+(j.usage?('<span style="color:var(--grey)">· $'+j.usage.est_cost_usd+'</span>'):'')+'</div>'
...
function disputeComposeBox(c, d, to){
  return '<div style="border:1px solid #e5c9c6;border-radius:9px;padding:9px;margin-bottom:10px;background:#fffdfd">'
    ...
    +'<textarea id="droom-'+d.dispute_id+'" data-testid="dispute-room-input" oninput="window.CBOffline&&CBOffline.saveDraft(\'disp.room.'+d.dispute_id+'\',this.value)" placeholder="Message — '+to+' will see this" style="width:100%;box-sizing:border-box;min-height:46px;border:1px solid var(--line);border-radius:8px;padding:7px;font:inherit;font-size:13px;resize:vertical">'+esc((window.CBOffline&&CBOffline.loadDraft('disp.room.'+d.dispute_id))||'')+'</textarea>'
    +'<div style="display:flex;align-items:center;gap:8px;margin-top:7px;flex-wrap:wrap">'
      ...
      +'<button data-testid="dispute-room-send" onclick="sendDisputeMsg(\''+c.id+'\',\''+d.dispute_id+'\')" style="margin-left:auto;background:#b4453f;color:#fff;border:none;border-radius:8px;padding:7px 14px;font-weight:600;cursor:pointer">Send ↔</button></div></div>';
}
