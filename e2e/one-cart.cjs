 ...
 *   – only app/cart-ui.js may build the "Total incl. tax" row or a per-rate tax table (`byRate`);
 *   – only app/cart-ui.js and app/offers.js may call CBOffers.evaluate for a BASKET (the product page's single-line preview
...
console.log('\n══ ONE CART ══');
console.log(bad ? '  ' + bad + ' second implementation(s) found\n' : '  ✓ money is computed in one place (app/cart-ui.js); every screen delegates\n');
