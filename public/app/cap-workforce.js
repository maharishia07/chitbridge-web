...
function openShowcase(url, title){ if(typeof modal!=='function') return; modal('<div class="mhd"><div class="t">'+esc(title||'How it works')+'</div></div><div class="mbody" style="padding:0"><iframe src="'+esc(url)+'" title="'+esc(title||'')+'" style="width:100%;height:72vh;border:0;display:block;background:#f4f6f8"></iframe></div>'); }
  ...
  const list = `<div class="list">
    <div class="lh">
      ...
      <div style="display:flex;align-items:center;gap:6px;margin-top:8px;font-size:11px;color:var(--grey);flex-wrap:wrap">
        ...
        <span style="display:inline-flex;border:1px solid var(--line);border-radius:8px;overflow:hidden">${['all'].concat(Object.keys(window.ACTOR_TYPES||{})).map(t=>{var reg=(window.ACTOR_TYPES||{})[t]||{};var on=(UI.acTypeF||'all')===t;var lbl=t==='all'?'All':(reg.icon||t);return `<button onclick="setAcTypeF('${t}')" title="${t==='all'?'all types':(reg.label||t)}${reg.comingSoon?' — coming soon':''}" style="border:0;background:${on?'var(--blue)':'#fff'};color:${on?'#fff':'var(--grey)'};font-weight:700;font-size:11px;padding:4px 9px${reg.comingSoon?';opacity:.55':''}">${lbl}</button>`;}).join('')}</span>${(UI.acTypeF==='iot')?`<a href="#" onclick="openShowcase('/iot-howitworks.html','How IoT works');return false" style="font-size:11px;color:var(--blue);font-weight:600;text-decoration:none;white-space:nowrap">❔ How it works</a>`:''}
