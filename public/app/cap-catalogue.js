...
function _cwStep2(w){
  ...
  var modeToggle = w.source ? '<div style="margin-top:10px;font-size:11.5px;color:#3a4048">Adopt the details <b>by</b>: <span onclick="cwSetAdoptMode(\'reference\')" style="cursor:pointer;font-weight:700;padding:3px 10px;border-radius:12px;border:1px solid ' + (mode === 'reference' ? '#2c7a43' : 'var(--line)') + ';color:' + (mode === 'reference' ? '#fff' : 'var(--grey)') + ';background:' + (mode === 'reference' ? '#2c7a43' : '#fff') + '">reference</span> <span onclick="cwSetAdoptMode(\'value\')" style="cursor:pointer;font-weight:700;padding:3px 10px;border-radius:12px;border:1px solid ' + (mode === 'value' ? '#2c5aa0' : 'var(--line)') + ';color:' + (mode === 'value' ? '#fff' : 'var(--grey)') + ';background:' + (mode === 'value' ? '#2c5aa0' : '#fff') + '">value</span><div style="font-size:10.5px;color:var(--grey);margin-top:4px">' + (mode === 'reference' ? '📎 Linked to the source — design stays shared, you store no copy (recommended).' : '📋 A copy of the details is stored in your catalogue — you own a snapshot.') + '</div></div>' : '';
  var items = (w.built && w.built.finishes) ? '<div style="margin-top:12px;display:flex;align-items:center;gap:8px"><span style="font-size:11px;font-weight:800;color:#6a44a8;letter-spacing:.05em">ITEMS FROM THE BLUEPRINT</span><span style="font-size:10px;color:var(--grey)">— pick what you sell</span><span onclick="cwChooseAll(true)" style="cursor:pointer;font-size:10px;color:var(--blue);margin-left:auto">all</span><span style="color:#c8d0d9">·</span><span onclick="cwChooseAll(false)" style="cursor:pointer;font-size:10px;color:var(--blue)">none</span></div>' + w.built.finishes.map(function(it){ var on = w.chosen[it.name] !== false; return '<div style="display:flex;align-items:center;gap:9px;padding:6px 10px;border:1px solid var(--line);border-radius:9px;margin-top:5px;background:#fff"><input type="checkbox" ' + (on ? 'checked' : '') + ' onchange="cwToggleItem(\'' + esc(it.name).replace(/'/g, "\\'") + '\')"><span style="font-weight:600;font-size:12.5px">' + esc(it.name) + '</span><span style="font-size:10.5px;color:var(--grey)">' + esc(it.texture_family || '') + ' · ' + esc(it.sheen || '') + '</span></div>'; }).join('') + '<div style="font-size:10.5px;color:#2c7a43;margin-top:8px">✓ These fields flow into your field list (step 1): ' + _cwBpFields(w).join(' · ') + '</div>' : '';
  return '<div style="font-size:13px;color:#3a4048;margin-bottom:10px">Is there a <b>blueprint</b> to adopt? A blueprint is a nicely-structured catalogue of this sort — pick the items you sell; their details come <b>by reference</b> (kept inside, no copy). <b>No blueprint? Skip.</b></div>' + picker + rights + modeToggle + items;
}
function _cwStep3(w){
...
function _cwMergeBpFields(w){ w.fieldSel = w.fieldSel || []; var have = {}; w.fieldSel.forEach(function(f){ have[f.name.toLowerCase()] = f; }); _cwBpFields(w).forEach(function(name){ var e = have[name.toLowerCase()]; if (e) { e.on = true; e._bp = true; } else { var nf = { section: 'From blueprint', name: name, type: 'text', on: true, leg: 'cb', _bp: true }; w.fieldSel.push(nf); have[name.toLowerCase()] = nf; } }); }
