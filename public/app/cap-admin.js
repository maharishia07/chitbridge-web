...
function standardsSettingsHTML(){
  ...
  if (tab === 'platform') {
    ...
    body = card('<div style="font-size:var(--fs-2);line-height:1.65;color:var(--on-card)">'
        + '<b>A chit crosses a boundary.</b> It leaves one company and lands in another that shares no system '
        + 'with it. Every convention we invent is one the other side has to be taught; every standard we adopt '
        + 'arrives already legible.'
        + '<div style="margin-top:8px;color:var(--grey)">So this is not hygiene around the rail — it is what '
        + 'makes a rail possible <b>without an integration project per counterparty</b>. That is the whole '
        + 'argument, and everything below is its detail.</div>'
        + '</div>')
      + col('What it buys', STD_WHY.pleasure, 'var(--ok-tint)', 'var(--ok-2)')
      + col('What it costs', STD_WHY.pain, 'var(--warn-tint)', 'var(--warn-2)')
      /* ⚠️ EVIDENCE FROM THIS CODEBASE, not assertions — each found BY adopting the standard, and none of them
         findable by careful work alone. This is the part that turns an opinion into a case. */
      + card('<div style="font-size:var(--fs-1);font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--grey);margin-bottom:6px">What it has actually caught here</div>'
