...
async function ok(page, observed) {
  ...
  if (process.env.TOUR_ME_PROBE) { const t0 = Date.now(); const r = await Promise.race([page.evaluate(async () => { const s = JSON.parse(localStorage.getItem("cb_sess") || "null"); const t1 = Date.now(); const r = await fetch((CFG.API_BASE || "") + "/api/entities/me?include=readiness,channels,vault", { headers: { Authorization: "Bearer " + (s && s.token) } }); return r.status + " " + (Date.now() - t1) + "ms"; }).catch((e) => "ERR " + String(e.message).slice(0, 80)), new Promise((ok) => setTimeout(() => ok("HUNG >20s"), 20000))]); console.log("[tour]   /me?readiness → " + r + " (" + (Date.now() - t0) + "ms)"); }
