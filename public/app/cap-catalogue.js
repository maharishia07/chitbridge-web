...
function catfFieldMapHTML(){
  ...
  var rows = keys.map(function(k){
    ...
    return '<tr style="border-bottom:1px solid var(--line)">'
      + '<td style="padding:6px 8px 6px 0;font-weight:700;font-size:var(--fs-1);white-space:nowrap;vertical-align:top">' + esc(k) + '</td>'
      + '<td style="padding:6px 8px;font-size:var(--fs-1);vertical-align:top">' + cell(f.s) + '</td>'
      + '<td style="padding:6px 8px;font-size:var(--fs-1);vertical-align:top">' + cell(f.m) + '</td>'
      + '<td style="padding:6px 8px;font-size:var(--fs-1);vertical-align:top;color:var(--blue-2)">' + cell(f.o) + '</td>'
      + '<td style="padding:6px 0 6px 8px;font-size:var(--fs-1);color:' + (warn ? 'var(--warn-2)' : 'var(--grey)') + ';line-height:1.45;vertical-align:top">' + esc(f.n || '') + '</td>'
      + '</tr>';
  }).join('');
  return hdr('FIELD BY FIELD', 'ours → theirs')
    ...
    + ['ours', 'schema.org', 'Medusa', 'other', 'note'].map(function(h){
        return '<th style="text-align:start;padding:0 8px 5px 0;font-size:var(--fs-1);font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--grey)">' + h + '</th>'; }).join('')
    ...
    + GAPS.map(function(g){ return '<div style="padding:7px 0;border-bottom:1px dashed var(--line)">'
        + '<span style="font-size:var(--fs-1);font-weight:700">' + esc(g.key) + '</span>'
        + '<span style="font-size:var(--fs-1);color:var(--grey)"> · ' + esc(g.from) + '</span>'
        + '<div style="font-size:var(--fs-1);color:var(--grey);line-height:1.5;margin-top:2px">' + esc(g.why) + '</div></div>'; }).join('')
    ...
    + CON.map(function(c){ return '<div style="padding:8px 0;border-bottom:1px dashed var(--line)">'
        + '<div style="font-size:var(--fs-1);font-weight:700">' + esc(c.ours) + '</div>'
        + '<div style="font-size:var(--fs-1);color:var(--grey);line-height:1.5;margin-top:2px">' + esc(c.issue) + '</div>'
        + '<div style="font-size:var(--fs-1);color:var(--blue-2);margin-top:3px">standard: ' + esc(c.standard) + '</div>'
        + '<div style="font-size:var(--fs-1);color:var(--warn-2);margin-top:2px">cost: ' + esc(c.cost) + '</div>'
...
function catfStandardsModal(){
  ...
  var row = function(s){ return '<div style="padding:10px 0;border-bottom:1px solid var(--line)">'
    + '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="font-size:var(--fs-2);font-weight:700;color:var(--ink-2)">' + esc(s.name) + '</span>' + badge(s.status) + '<span style="font-size:var(--fs-1);color:var(--grey)">' + esc(s.body) + '</span></div>'
...
function catfPublishBlueprint(){
  ...
  api('catSourcePut', { body: body }).then(function(r){
    if (r && r.ok) {
      ...
      { up: '<b>own unit + price</b>', ton: '<b>ton</b>', kg: '<b>kg</b>', byref: '<b>by reference</b>' }) + '</div></div></div>', true);
...
function catfFillItem(){
  ...
  _catfLoadJE().then(function(){
    setTimeout(function(){ var el = document.getElementById('cat_je'); if (!el) return; el.innerHTML = '';
      ...
      } catch (e) { el.innerHTML = '<div style="color:var(--disp-2);font-size:var(--fs-2)">json-editor failed: ' + esc(e.message) + '</div>'; }
    }, 40);
  }).catch(function(){ var el = document.getElementById('cat_je'); if (el) el.innerHTML = '<div style="color:var(--disp-2);font-size:var(--fs-2)">Could not load the json-editor library (/app/vendor/json-editor.min.js).</div>'; });
}
...
function catfCustomerPreview(){
  ...
  if (typeof modal === 'function') modal('<div class="mhd"><div class="t">' + tx('👁 Customer experience — end to end') + '</div></div><div class="mbody" style="padding:0"><div style="padding:14px 18px;max-height:72vh;overflow:auto">' + _catfAppearsTab(tmp, c, facets) + '</div></div>', true);
}
...
function _catfWizard(){
  ...
  var nav = '<div style="display:flex;gap:10px;margin-top:20px;align-items:center">'
    ...
    + (step < 6 ? '<button class="pri" onclick="cwNext()" style="padding:9px 18px">' + tx('Next ›') + '</button>' : '<button class="pri" onclick="cwFinish()" style="padding:9px 18px">' + tx('✓ Finish — go live') + '</button>')
    + (step < 6 ? '<span onclick="cwFinish()" style="cursor:pointer;font-size:var(--fs-1);color:var(--blue)">or finish now (partial is fine)</span>' : '')
    + '<span onclick="cwCancel()" style="cursor:pointer;font-size:var(--fs-1);color:var(--grey);margin-inline-start:auto">' + tx('Cancel') + '</span></div>';
  return '<div style="flex:1;min-height:0;overflow-y:auto;padding-bottom:var(--scroll-tail);padding:22px 22px var(--scroll-tail)"><div style="max-width:940px"><div style="font-size:var(--fs-5);font-weight:800">' + tx('🗂️ Set up your catalogue') + '</div><div style="font-size:var(--fs-1);color:var(--grey);margin:4px 0 16px">Currency <b>' + esc(_catfCcy()) + '</b> — from Settings · fill as much as you can, in order; each step is optional.</div>' + bar + body + nav + '</div></div>';
}
/* two-panel wrapper + the running "catalogue so far" preview (the right side, shared across steps 2–6) */
function _cwTwo(left, right){ return '<div style="display:flex;gap:18px;flex-wrap:wrap"><div style="flex:0 0 46%;min-width:280px;max-width:410px">' + left + '</div><div style="flex:1;min-width:260px;border-inline-start:1px solid var(--line);padding-inline-start:18px">' + right + '</div></div>'; }
function _cwPreview(w){
  ...
  var head = '<div style="font-size:var(--fs-1);font-weight:800;color:var(--blue-2);letter-spacing:.05em">' + tx('YOUR CATALOGUE SO FAR') + '</div>'
  ...
  var fieldsHtml = fields.length ? '<div style="margin-top:11px;font-size:var(--fs-1);font-weight:700;color:var(--grey-2);text-transform:uppercase;letter-spacing:.04em">' + tx('Fields') + '</div>' + fields.map(function(f){ var src = cov[_cwNorm(f.name)]; var b = src === 'blueprint' ? ['📎 blueprint', 'var(--purple-2)'] : src === 'erp' ? ['🔗 ERP', 'var(--warn-2)'] : src === 'manual' ? ['✍ you', 'var(--ok-2)'] : ['· to fill', 'var(--grey-4)']; return '<div style="display:flex;align-items:center;gap:6px;font-size:var(--fs-1);padding:1px 0"><span style="flex:1;color:var(--ink-2)">' + esc(f.name) + '</span><span style="font-size:var(--fs-1);font-weight:700;color:' + b[1] + ';background:' + b[1] + '18;border-radius:4px;padding:1px 6px">' + b[0] + '</span></div>'; }).join('') : '';
  var itemsHtml = items.length ? '<div style="margin-top:11px;font-size:var(--fs-1);font-weight:700;color:var(--grey-2);text-transform:uppercase;letter-spacing:.04em">Items · ' + items.length + '</div>' + items.slice(0, 7).map(function(it){ var p = w.prices[it.name]; return '<div style="display:flex;align-items:center;gap:6px;font-size:var(--fs-1);padding:1px 0"><span style="flex:1">' + esc(it.name) + '</span><span style="font-weight:600;color:' + (p != null && p !== '' ? 'var(--blue-2)' : 'var(--disp-2)') + '">' + (p != null && p !== '' ? esc(_catfMoney(p)) : 'no price') + '</span></div>'; }).join('') : '';
...
function _cwMethodBlock(w){
  ...
  var chip = function(m){ var on = m.k === cur;
    return '<span data-testid="cw-method-' + esc(m.k) + '" onclick="cwSetMethod(\'' + esc(m.k) + '\')" title="' + esc(m.hint) + '" style="cursor:pointer;font-size:var(--fs-1);font-weight:600;padding:3px 10px;border-radius:13px;border:1px solid ' + (on ? 'var(--blue-2)' : 'var(--line)') + ';color:' + (on ? 'var(--on-accent)' : 'var(--grey)') + ';background:' + (on ? 'var(--blue-2)' : 'var(--card)') + '">' + (on ? '✓ ' : '') + esc(m.label) + '</span>'; };
  return '<div style="margin-bottom:14px">'
    ...
    + '<div style="margin-top:6px;font-size:var(--fs-1);color:var(--blue-2);background:var(--blue-tint-bg);border:1px solid var(--blue-tint-line);border-radius:6px;padding:5px 9px">'
    ...
    right = '<div style="font-size:var(--fs-1);font-weight:800;color:var(--blue-2);letter-spacing:.05em;margin-bottom:8px">FIELDS TO STORE <span style="font-weight:500;color:var(--grey)">— pick what this catalogue keeps</span></div>'
      + (function(){ var us = w.units || []; var chip = function(u, known){ var on = us.indexOf(u) >= 0; return '<span onclick="cwToggleUnit(\'' + esc(u).replace(/'/g, "\\'") + '\')" style="cursor:pointer;font-size:var(--fs-1);font-weight:600;padding:3px 10px;border-radius:13px;border:1px solid ' + (on ? 'var(--ok-2)' : 'var(--line)') + ';color:' + (on ? 'var(--on-accent)' : 'var(--grey)') + ';background:' + (on ? 'var(--ok-2)' : 'var(--card)') + '">' + (on ? '✓ ' : '') + esc(u) + (on && !known ? ' ×' : '') + '</span>'; };
        ...
        return '<div style="margin-bottom:14px"><div style="font-size:var(--fs-1);color:var(--grey);margin-bottom:5px">Sold by <span style="color:var(--grey-4)">— pick every unit this catalogue uses (products can differ: kg · litre · count…)</span></div>'
          ...
          + '<div style="display:flex;gap:6px;margin-top:7px"><input id="cw_newunit" placeholder="add a unit (e.g. drum, coil)" style="width:170px;padding:4px 8px;border:1px solid var(--line);border-radius:6px;font-size:var(--fs-1)"><button onclick="cwAddUnit()" style="padding:4px 11px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--blue);font-weight:600;font-size:var(--fs-1);cursor:pointer">' + tx('Add') + '</button></div></div>'; })()
      + _cwMethodBlock(w)
      + order.map(function(sec){ return '<div style="margin-bottom:11px"><div style="font-size:var(--fs-1);font-weight:700;color:var(--grey-2);text-transform:uppercase;letter-spacing:.04em;margin-bottom:2px">' + esc(sec) + '</div>' + secs[sec].map(function(f){ var idx = w.fieldSel.indexOf(f); return '<label style="display:flex;align-items:center;gap:8px;padding:2px 0;cursor:pointer"><input type="checkbox" ' + (f.on ? 'checked' : '') + ' onchange="cwToggleField(' + idx + ')"><span style="font-size:var(--fs-2);font-weight:' + (f.on ? 600 : 400) + ';color:' + (f.on ? 'var(--blue-2)' : 'var(--grey)') + '">' + esc(f.name) + '</span><span style="font-size:var(--fs-1);color:var(--grey-4);background:var(--blue-tint-bg);border-radius:4px;padding:1px 6px">' + esc(f.type) + '</span>' + (f._bp ? '<span style="font-size:var(--fs-1);color:var(--purple-2)" title="from blueprint">📎</span>' : f._added ? '<span style="font-size:var(--fs-1);color:var(--ok-2)" title="you added">＋</span>' : '') + '</label>'; }).join('') + '</div>'; }).join('')
      ...
      + '<div style="margin-top:8px"><div style="font-size:var(--fs-1);font-weight:700;color:var(--grey-2);text-transform:uppercase;letter-spacing:.04em">' + tx('Add a data type') + '</div><div style="display:flex;gap:6px;margin-top:5px"><input id="cw_newfield" placeholder="field name" style="flex:1;min-width:0;padding:5px 8px;border:1px solid var(--line);border-radius:6px;font-size:var(--fs-2)"><select id="cw_newtype" style="padding:5px;border:1px solid var(--line);border-radius:6px;font-size:var(--fs-1)">' + ['text', 'number', 'choice', 'date', 'boolean'].map(function(t){ return '<option>' + t + '</option>'; }).join('') + '</select><button onclick="cwAddCustomField()" style="padding:5px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--blue);font-weight:600;font-size:var(--fs-1);cursor:pointer">' + tx('Add') + '</button></div><div style="font-size:var(--fs-1);color:var(--grey);margin-top:4px">Uncheck any you don\'t need · add any that are missing. Adopting a blueprint (step 2) adds its fields here too 📎.</div></div>';
  } else {
...
function _cwStep2Fields(w){
  ...
  if (!fields.length) return head + '<div style="font-size:var(--fs-2);color:var(--grey);margin-top:8px">Pick fields in step 1 first. Adopting a blueprint (right) adds its fields here.</div>';
  var rows = fields.map(function(f){ var v = sel ? sel[f._bpKey || f.name] : undefined; var vstr = (v == null) ? '' : (Array.isArray(v) ? v.map(function(x){ return (x && typeof x === 'object') ? (x.name || '') : x; }).join(', ') : (typeof v === 'object' ? (v.name || JSON.stringify(v)) : String(v))); return '<div style="display:flex;gap:8px;padding:3px 0;border-bottom:1px dashed var(--line);font-size:var(--fs-1)"><span style="flex:0 0 148px;color:var(--ink-2)">' + esc(f.name) + (f._bp ? ' <span style="color:var(--purple-2);font-size:var(--fs-1)">📎</span>' : f._added ? ' <span style="color:var(--ok-2);font-size:var(--fs-1)">＋</span>' : '') + '</span><span style="flex:1;color:' + (vstr ? 'var(--blue-2)' : 'var(--grey-4)') + '">' + (sel ? (vstr ? esc(vstr) : '—') : '<span style="font-size:var(--fs-1)">value shows when you click a product ›</span>') + '</span></div>'; }).join('');
  ...
  var summary = w.source ? '<div style="margin-top:6px;font-size:var(--fs-1);line-height:1.6;border-bottom:1px solid var(--line);padding-bottom:8px">'
    ...
    + '</div>' : '';
  var swatches = (sel && sel.combinations && sel.combinations.length) ? '<div style="margin-top:10px;border-top:1px solid var(--line);padding-top:8px"><div style="font-size:var(--fs-1);font-weight:700;color:var(--grey-2);text-transform:uppercase;letter-spacing:.04em">' + tx('Colour combinations') + '</div>' + sel.combinations.map(_cwCombo).join('') + '</div>' : '';
...
function _cwSwatch(c){ return '<span title="' + esc((c.name || '') + ' ' + (c.hex || '')) + '" style="display:inline-block;width:15px;height:15px;border-radius:50%;background:' + esc(c.hex || 'var(--neutral-tint)') + ';border:1px solid rgba(0,0,0,.15);vertical-align:middle;margin-inline-end:2px"></span>'; }
...
function _cwProductCard(w, it){
  ...
  var chips = [it.texture_family, it.region].filter(Boolean).map(function(c){ return '<span style="font-size:var(--fs-1);background:' + accent + '18;color:' + accent + ';border-radius:4px;padding:1px 6px">' + esc(c) + '</span>'; }).join('') + (it.effect || []).map(function(e){ return '<span style="font-size:var(--fs-1);background:var(--warn-tint);color:var(--warn-3);border-radius:4px;padding:1px 6px">' + esc(e) + '</span>'; }).join('');
...
function _cwStep2Products(w){
  ...
  var picker;
  if (srcs === null) picker = '<div style="color:var(--grey);font-size:var(--fs-2)">loading blueprints…</div>';
  else {
    ...
    var skipRow = '<div onclick="cwPickSource(\'\')" style="cursor:pointer;padding:7px 11px;border-bottom:1px solid var(--line);background:' + (!w.source ? 'var(--card)' : 'var(--card)') + ';font-size:var(--fs-2);color:var(--grey)">— skip (build without a blueprint) —</div>';
    var rows = filtered.map(function(s){ var on = w.source === s.key;
      return '<div onclick="cwPickSource(\'' + esc(s.key) + '\')" style="cursor:pointer;padding:8px 11px;border-bottom:1px solid var(--line);background:' + (on ? 'var(--blue-tint-bg)' : 'var(--card)') + ';color:' + (on ? 'var(--on-card)' : 'var(--on-card)') + '">'
        ...
        + '<span style="margin-inline-start:auto;font-size:var(--fs-1);color:var(--grey)">' + s.item_count + ' item(s)</span></div>'
        + (s.for_entity ? '<div style="font-size:var(--fs-1);color:var(--grey-4);margin-top:1px">' + esc(s.for_entity) + '</div>' : '') + '</div>'; }).join('');
    var empty = '<div style="font-size:var(--fs-1);color:var(--grey);padding:9px 11px">No blueprints' + (q ? ' match “' + esc(q) + '”' : ((w.vertical && !w.bpAll) ? ' for ' + esc(w.vertical) : '')) + '.' + ((otherCount && !w.bpAll) ? ' <span onclick="cwToggleBpAll()" style="cursor:pointer;color:var(--blue);font-weight:600">Show all verticals.</span>' : '') + '</div>';
  ...
  var rights = w.source ? '<div style="margin-top:6px;font-size:var(--fs-1);color:var(--ok-2)">🔓 Rights ok <span style="color:var(--grey)">— you may use this blueprint (distributor grant)</span></div>' : '';
  var mode = w.adoptMode || 'reference';
  var modeToggle = w.source ? '<div style="margin-top:8px;font-size:var(--fs-1);color:var(--ink-2)">by: <span onclick="cwSetAdoptMode(\'reference\')" style="cursor:pointer;font-weight:700;padding:2px 9px;border-radius:12px;border:1px solid ' + (mode === 'reference' ? 'var(--ok-2)' : 'var(--line)') + ';color:' + (mode === 'reference' ? 'var(--on-accent)' : 'var(--grey)') + ';background:' + (mode === 'reference' ? 'var(--ok-2)' : 'var(--card)') + '">reference</span> <span onclick="cwSetAdoptMode(\'value\')" style="cursor:pointer;font-weight:700;padding:2px 9px;border-radius:12px;border:1px solid ' + (mode === 'value' ? 'var(--blue-2)' : 'var(--line)') + ';color:' + (mode === 'value' ? 'var(--on-accent)' : 'var(--grey)') + ';background:' + (mode === 'value' ? 'var(--blue-2)' : 'var(--card)') + '">value</span></div>' : '';
  var products = (w.built && w.built.finishes) ? '<div style="margin-top:12px;display:flex;align-items:center;gap:8px"><span style="font-size:var(--fs-1);font-weight:800;color:var(--purple-2);letter-spacing:.05em">PRODUCTS · ' + w.built.finishes.length + '</span><span style="font-size:var(--fs-1);color:var(--grey)">tick to sell · click to inspect</span><span onclick="cwChooseAll(true)" style="cursor:pointer;font-size:var(--fs-1);color:var(--blue);margin-inline-start:auto">all</span><span style="color:#c8d0d9">·</span><span onclick="cwChooseAll(false)" style="cursor:pointer;font-size:var(--fs-1);color:var(--blue)">none</span></div>' + w.built.finishes.map(function(it){ return _cwProductCard(w, it); }).join('') : '';
  return '<div style="font-size:var(--fs-2);color:var(--ink-2);margin-bottom:8px">' + txf('Pick a {bp} — a ready, structured catalogue of this sort. Its {products} list here; tick the ones you sell, click one to see its field values on the left. {skip}',
      { bp: '<b>blueprint</b>', products: '<b>products</b>', skip: '<b>No blueprint? Skip.</b>' }) + '</div>' + picker + rights + modeToggle + products;
}
function _cwStep3(w){
  var fields = (w.fieldSel || []).filter(function(f){ return f.on; });
...
function _cwStep4(w){
  ...
  var seg = function(m, l){ var on = mode === m; return '<span onclick="cwBulkMode(\'' + m + '\')" style="cursor:pointer;font-size:var(--fs-1);font-weight:600;padding:5px 11px;border-radius:13px;border:1px solid ' + (on ? 'var(--blue-2)' : 'var(--line)') + ';color:' + (on ? 'var(--on-accent)' : 'var(--grey)') + ';background:' + (on ? 'var(--blue-2)' : 'var(--card)') + '">' + l + '</span>'; };
  ...
  if (mode === 'csv') {
    ...
    body = '<div style="font-size:var(--fs-2);color:var(--ink-2);margin-bottom:8px">' + txf('Only have {photos}? Add the pictures — each becomes an item, and you confirm the name and price.', { photos: '<b>' + tx('photos or product labels') + '</b>' }) + specNote('catalogue.photos.confirm', 'The same human-confirm step the <b>Capture</b> connector uses.') + '</div>'
      ...
      + (ph.length ? '<button data-testid="cw-photo-read" onclick="cwPhotosRead()" ' + (w._phBusy ? 'disabled ' : '')
          + 'style="margin-inline-start:8px;padding:8px 15px;border:1px solid var(--purple);border-radius:9px;background:var(--card);color:var(--purple);font-weight:600;cursor:pointer">'
          + (w._phBusy ? '✨ Reading…' : '✨ Read the labels') + '</button>' : '')
      + (w._phErr ? '<div style="margin-top:8px;background:var(--danger-tint);border:1px solid #f0c9c6;border-radius:9px;padding:8px 11px;font-size:var(--fs-2);color:var(--disp)">' + esc(w._phErr) + '</div>' : '')
      + (w._phNote ? '<div style="margin-top:8px;background:var(--blue-tint-bg);border:1px solid #e4dff6;border-radius:9px;padding:8px 11px;font-size:var(--fs-2);color:var(--blue)">' + esc(w._phNote) + '</div>' : '')
      + (ph.length ? '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(148px,1fr));gap:10px;margin-top:12px">'
          + ph.map(function(p){ return '<div style="border:1px solid var(--line);border-radius:9px;padding:8px;background:var(--card);color:var(--on-card)">'
            + '<div style="height:88px;background:var(--card) center/cover no-repeat;background-image:url(' + p.url + ');border-radius:6px;color:var(--on-card)"></div>'
            /**
             ...
             */
            + (p._ai ? '<div style="margin-top:6px;font-size:var(--fs-1);font-weight:800;color:var(--purple);background:var(--purple-tint);border-radius:5px;padding:1px 6px;display:inline-block">' + tx('✨ proposed — check it') + '</div>' : '')
            + '<input value="' + esc(p.name || '') + '" oninput="cwSetPhotoField(\'' + p.id + '\',\'name\',this.value)" placeholder="item name" style="width:100%;box-sizing:border-box;margin-top:6px;padding:5px 7px;border:1px solid ' + (p._ai ? 'var(--blue-tint-bg)' : 'var(--line)') + ';border-radius:6px;font-size:var(--fs-1)">'
            + '<div style="display:flex;gap:5px;margin-top:5px;align-items:center"><span style="font-size:var(--fs-1);color:var(--grey)">' + esc(_catfCcy()) + '</span><input type="number" value="' + (p.price != null ? p.price : '') + '" oninput="cwSetPhotoField(\'' + p.id + '\',\'price\',this.value)" placeholder="price" style="flex:1;min-width:0;padding:4px 6px;border:1px solid var(--line);border-radius:6px;font-size:var(--fs-1)"><span onclick="cwPhotoRemove(\'' + p.id + '\')" style="cursor:pointer;color:var(--disp-2);font-size:15px;line-height:1" title="remove">×</span></div>'
            + '</div>'; }).join('') + '</div>'
          ...
          + (committed ? '<div style="margin-top:7px;font-size:var(--fs-1);color:var(--ok-2)">✓ ' + committed + ' photo item(s) in your catalogue.</div>' : '')
        : '')
...
function _cwUnitSelect(units, sel, onchange){
  return '<select data-testid="cw-item-unit" onchange="' + onchange + '" title="unit this item sells by" style="padding:5px 6px;border:1px solid var(--line);border-radius:6px;font-size:var(--fs-2);background:var(--card);color:var(--ink-2)">'
...
function _cwStep6(w){
  ...
  return '<div style="font-size:var(--fs-2);color:var(--ink-2);margin-bottom:10px">' + txf('Set {tax}, then go live.', { tax: '<b>' + tx('tax') + '</b>' }) + '</div>'
    ...
    + '<div style="margin-top:10px;padding:11px 13px;border:1px solid #cfe6cf;border-radius:9px;background:var(--ok-tint);font-size:var(--fs-2);color:var(--ok-2)">' + txf('On finish: your business uses the blueprint plus your {n}.', { n: '<b>' + priced + ' ' + tx('price(s)') + '</b>' }) + '' + (w.source ? '' : ' — <i>no blueprint, so it saves your manual items</i>') + txf(' It goes {live} on your storefront. Half-filled is fine — you can add the rest anytime.', { live: '<b>' + tx('live') + '</b>' }) + '</div>';
}
/* wizard actions */
...
function _catfSettingsNote(){ return 'Currency <b>' + esc(_catfCcy()) + '</b> · country <b>' + esc(_catfCountry()) + '</b> — from <b>' + tx('Settings') + '</b> (set at registration), inherited here.'; }
...
function _catfMethodControl(method, price){
  ...
  var btn = 'background:var(--blue-2);color:var(--on-accent);border-radius:5px;padding:3px 10px;font-size:var(--fs-1);font-weight:600';
  if (method === 'text') return '<span style="font-size:var(--fs-1);color:var(--grey)">information only</span>';
  if (method === 'cart') return '<span style="font-size:var(--fs-1);color:var(--grey)">Qty ▢ × ' + esc(p) + '</span> <span style="' + btn + '">' + tx('Add') + '</span>';
  if (method === 'range') return '<span style="font-size:var(--fs-1);color:var(--grey)">' + esc(_catfMoney(3200)) + ' – ' + esc(_catfMoney(3600)) + '</span> <span style="' + btn + '">' + tx('Order') + '</span>';
  if (method === 'qty') return '<span style="font-size:var(--fs-1);color:var(--grey)">' + tx('Qty ▢') + '</span> <span style="' + btn + '">' + tx('Order') + '</span>';
  return '<span style="font-size:var(--fs-1);color:var(--grey)">' + tx('Qty ▢ · your price ▢') + '</span> <span style="' + btn + '">' + tx('Offer') + '</span>';
}
...
function _catfAppearsTab(f, c, facets){
  ...
  var step = function(n, title, inner){ return '<div style="border:1px solid var(--line);border-radius:12px;background:var(--card);padding:11px 13px;margin-top:8px;color:var(--on-card)"><div style="font-size:var(--fs-1);font-weight:800;color:var(--blue-2);letter-spacing:.04em;margin-bottom:8px">' + n + ' · ' + title + '</div>' + inner + '</div>'; };
  var arrow = '<div style="text-align:center;color:#c8d0d9;font-size:var(--fs-3);line-height:1.1">↓</div>';
  // 1 · browse (listing grid)
  var browse = '<div style="display:flex;gap:8px;flex-wrap:wrap">' + rows.map(function(r){ return '<div style="width:132px;border:1px solid var(--line);border-radius:9px;padding:8px 9px;background:var(--card);color:var(--on-card)">' + (facets.media ? '<div style="height:46px;border-radius:6px;background:linear-gradient(135deg,#eef1f5,#dde3ea);margin-bottom:6px"></div>' : '') + '<div style="font-weight:600;font-size:var(--fs-1);line-height:1.2">' + esc(r.name || 'item') + '</div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px"><span style="font-weight:700;font-size:var(--fs-1)">' + esc(_catfMoney(priceOf(r))) + '</span><span style="background:var(--blue-2);color:var(--on-accent);border-radius:4px;padding:2px 7px;font-size:var(--fs-1);font-weight:600">' + (f.method === 'cart' ? 'Add' : 'View') + '</span></div></div>'; }).join('') + '</div>';
  // 2 · product detail
  var p0 = rows[0] || {};
  ...
  var detail = '<div style="display:flex;gap:11px">' + (facets.media ? '<div style="width:86px;height:86px;border-radius:9px;background:linear-gradient(135deg,#eef1f5,#dde3ea);flex:none"></div>' : '') + '<div style="flex:1;min-width:0"><div style="font-weight:700;font-size:var(--fs-3)">' + esc(p0.name || c.product || 'Item') + '</div><div style="font-weight:800;font-size:var(--fs-3);margin-top:2px">' + esc(_catfMoney(priceOf(p0))) + (p0.unit ? ' <span style="font-size:var(--fs-1);color:var(--grey);font-weight:400">/ ' + esc(p0.unit) + '</span>' : '') + '</div>' + (spec ? '<div style="margin-top:8px">' + spec + '</div>' : '') + '<div style="margin-top:10px">' + _catfMethodControl(f.method, p0.price) + '</div></div></div>';
  // 3 · cart & checkout
  var qty = 3; var up = parseFloat(priceOf(p0)) || 40; var tot = f.method === 'cart' ? up * qty : up;
  var cart = '<div style="display:flex;justify-content:space-between;font-size:var(--fs-2)"><span>' + esc(p0.name || 'Item') + ' × ' + qty + '</span><span style="font-weight:600">' + esc(_catfMoney(tot)) + '</span></div><div style="display:flex;justify-content:space-between;margin-top:6px;border-top:1px solid var(--line);padding-top:6px"><span style="font-weight:700">Total (' + esc(ccy) + ')</span><span style="font-weight:800">' + esc(_catfMoney(tot)) + '</span></div><div style="text-align:end;margin-top:9px"><span style="background:var(--ok-2);color:var(--on-ok);border-radius:6px;padding:5px 14px;font-size:var(--fs-1);font-weight:600">' + tx('Place order') + '</span></div>';
  ...
  var chit = '<div style="max-width:280px;margin:0 auto;border:1px solid var(--line);border-top:3px solid var(--blue-2);border-radius:9px;padding:11px 12px;background:var(--blue-tint-bg);color:var(--on-card)"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-weight:700;font-size:var(--fs-2)">' + esc(p0.name || 'Item') + '</span><span style="font-size:var(--fs-1);color:var(--ok-2);font-weight:700">' + tx('✓ placed') + '</span></div><div style="font-size:var(--fs-1);color:var(--grey);margin-top:3px">Qty ' + qty + (f.method === 'cart' ? ' · ' + esc(_catfMoney(tot)) : '') + (p0.unit ? ' ' + esc(p0.unit) : '') + '</div>' + (fieldLines ? '<div style="margin-top:6px;border-top:1px dashed var(--line);padding-top:5px">' + fieldLines + '</div>' : '') + '<div style="border-top:1px dashed var(--line);margin-top:8px;padding-top:5px;font-size:var(--fs-1);color:var(--faint,var(--grey-3));font-family:monospace">' + tx('🔒 sealed · arrives on the rail · both parties keep a copy') + '</div></div>';
  return '<div style="font-size:var(--fs-1);font-weight:800;color:var(--ok-2);letter-spacing:.05em">' + tx('🛍 CUSTOMER EXPERIENCE — end to end') + '</div>'
...
function _catfItemsHtml(f){
  ...
  var rows = items.map(function(it, i){
    ...
    var priceCell = (it.price != null && it.price !== '')
      ? '<span style="font-weight:700;font-size:var(--fs-2)">' + esc(_catfMoney(it.price)) + '</span> <span onclick="catfEditPrice(' + i + ')" style="cursor:pointer;color:var(--blue);font-size:var(--fs-1)">edit</span>'
      : '<input type="number" placeholder="set price" onchange="catfSetItemPrice(' + i + ',this.value)" title="the ONLY thing you enter for a referenced item" style="width:92px;padding:4px 7px;border:1px solid #d98b84;border-radius:6px;font-size:var(--fs-2);background:var(--danger-tint);color:var(--on-card)">';
    return '<div style="border:1px solid var(--line);border-radius:9px;padding:8px 11px;margin-top:5px;background:var(--card);color:var(--on-card)"><div style="display:flex;align-items:center;gap:8px">'
      + ((it._photo || it.photo) ? '<span style="width:26px;height:26px;border-radius:5px;background:var(--card) center/cover no-repeat;background-image:url(' + (it._photo || it.photo) + ');flex:none;color:var(--on-card)"></span>' : (it._media ? '<span style="width:22px;height:22px;border-radius:5px;background:linear-gradient(135deg,#eef1f5,#dde3ea);flex:none"></span>' : ''))
      ...
      + '<span style="margin-inline-start:auto;display:flex;align-items:center;gap:6px">' + _catfUnitCell(f, it, i) + priceCell + '</span></div>'
...
function _catfFaceView(){
  ...
  var facetRows = CATF_FACETS.map(function(x){ var on = !!facets[x.k];
    return '<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--line)">'
      + '<div style="flex:1"><div style="font-weight:600;font-size:var(--fs-2);color:' + (on ? 'var(--blue-2)' : 'var(--grey)') + '">' + esc(x.label) + '</div><div style="font-size:var(--fs-1);color:var(--grey)">' + esc(x.hint) + '</div></div>'
      + '<span onclick="catfToggleFacet(\'' + x.k + '\')" style="cursor:pointer;font-size:var(--fs-1);font-weight:700;padding:4px 12px;border-radius:14px;border:1px solid ' + (on ? 'var(--ok-2)' : 'var(--line)') + ';color:' + (on ? 'var(--on-accent)' : 'var(--blue)') + ';background:' + (on ? 'var(--ok-2)' : 'var(--card)') + '">' + (on ? '✓ on' : '＋ add') + '</span></div>';
  }).join('');
  var inner = '<div style="max-width:660px">'
    + '<div style="display:flex;align-items:center;gap:10px"><div style="font-size:var(--fs-4);font-weight:800">' + tx('🗂️ Catalogue face') + '</div>' + (f.adoptedFrom ? '<span style="font-size:var(--fs-1);font-weight:700;color:var(--blue-2);background:var(--blue-tint-bg);border-radius:5px;padding:2px 8px">adopted · ' + esc(f.adoptedFrom) + '</span>' : '<span style="font-size:var(--fs-1);font-weight:700;color:var(--purple-2);background:var(--purple-tint);border-radius:5px;padding:2px 8px">built from your data</span>') + '</div>'
    ...
    + ((f.units && f.units.length) || (c.altUnits && c.altUnits.length) ? '<div style="font-size:var(--fs-1);color:var(--grey);margin-top:10px">Sold by <span style="color:var(--ink-2);font-weight:600">' + esc((f.units && f.units.length) ? f.units.join(' · ') : [c.baseUnit].concat(c.altUnits || []).join(' · ')) + '</span> <span style="color:var(--faint,var(--grey-3))">— items may use any of these</span></div>' : '')
    ...
    + _catfItemsHtml(f)
    + '<div style="font-size:var(--fs-1);color:var(--grey);margin-top:16px">Item data comes three ways — <b>' + tx('📎 from the source') + '</b> (owner sets price only) · <b>' + tx('✍ entered') + '</b> · <b>' + tx('🔗 from ERP') + '</b>:</div>'
    + '<div style="display:flex;gap:10px;margin-top:6px;flex-wrap:wrap">'
    + '<button class="pri" onclick="catfFillItem()" style="padding:9px 15px">＋ Add manually</button>'
    + '<button onclick="catfSyncERP()" style="padding:9px 15px;border:1px solid #b07b1e;border-radius:9px;background:var(--card);color:var(--warn-2);font-weight:600">' + tx('🔗 From ERP') + '</button>'
    + '<button onclick="catfCustomerPreview()" style="padding:9px 15px;border:1px solid var(--ok-2);border-radius:9px;background:var(--card);color:var(--ok-2);font-weight:600">' + tx('👁 Customer experience') + '</button>'
    + '<button onclick="catfManage()" style="padding:9px 15px;border:1px solid var(--blue-2);border-radius:9px;background:var(--card);color:var(--blue-2);font-weight:600">' + tx('🗂️ Manage in Catalogue') + '</button>'
    + '<input id="catf_photo_input" type="file" accept="image/*" multiple style="display:none" onchange="catfAddPhotos(this)">'
    + '<button onclick="catfPhotoAttachBtn()" style="padding:9px 15px;border:1px solid var(--ok-2);border-radius:9px;background:var(--card);color:var(--ok-2);font-weight:600">' + tx('📷 Add photos') + '</button>'
    + '<button onclick="catfEnrichAI()" style="padding:9px 15px;border:1px solid #b07b1e;border-radius:9px;background:var(--card);color:var(--warn-2);font-weight:600">' + tx('✨ Enrich (AI)') + '</button>'
