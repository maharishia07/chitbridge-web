...
test('DEMO lifecycle: CSV products up → storefront order from another system → the order lands as a file', async ({ page, browser }) => {
  ...
  await test.step('2 · products up from the CSV', async () => {
    ...
    await page.screenshot({ path: path.join(OUT, 'lifecycle-1-catalogue-from-csv.png'), fullPage: false });
  });
  ...
  try {
    await test.step('3 · the storefront from another system (a phone, no login)', async () => {
      ...
      await shop.screenshot({ path: path.join(OUT, 'lifecycle-2-storefront-phone.png') });
    });
    await test.step('4 · the customer orders three lines', async () => {
      ...
      await shop.screenshot({ path: path.join(OUT, 'lifecycle-3-basket.png') });
      ...
      await shop.waitForTimeout(800); await shop.screenshot({ path: path.join(OUT, 'lifecycle-5-order-placed.png') });
    });
  ...
  await test.step('5 · the seller: the order on the Task list', async () => {
    ...
    await page.screenshot({ path: path.join(OUT, 'lifecycle-6-seller-task-list.png') });
    await page.evaluate((id) => openChit(id, true), chitId); await settle(page); await page.waitForTimeout(600);
    await page.screenshot({ path: path.join(OUT, 'lifecycle-7-seller-order-chit.png') });
  });
  ...
  await test.step('7 · Integrations shows the connector checked in', async () => {
    ...
    await page.screenshot({ path: path.join(OUT, 'lifecycle-9-integrations-running.png'), fullPage: true });
  });
