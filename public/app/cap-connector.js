...
function acHowItWorks(iot){
  ...
  var body = iot
    ? '<div style="font-weight:800;font-size:var(--fs-4);margin-bottom:8px">' + tx('🛰️ How an IoT connector works') + '</div><div style="font-size:var(--fs-2);color:var(--ink-2);line-height:1.6">' + txf('A Pi or gateway holds its own key and {pushes} readings to the rail. Each exception becomes a {chit} filed into your folder — the reading itself is the record that is kept.', { pushes: '<b>' + tx('pushes') + '</b>', chit: '<b>' + tx('co-held chit') + '</b>' }) + specNote('connector.iot-health', 'Health goes live by heartbeat.') + '</div>'
    : '<div style="font-weight:800;font-size:var(--fs-4);margin-bottom:8px">' + tx('🔌 How an ERP connector works') + '</div><div style="font-size:var(--fs-2);color:var(--ink-2);line-height:1.6">' + txf(
...
function _erpReceiptsHTML(){
  ...
  var rows=rs.map(function(r){
    ...
    return '<div style="display:flex;align-items:center;gap:9px;padding:9px 0;border-bottom:1px dashed var(--line);font-size:var(--fs-2)">'
      ...
      +'<span style="font-weight:700;font-size:var(--fs-1);color:'+col+'">'+esc(oc)+'</span>'
      +(r.chit_id?'<button class="composebtn" style="padding:2px 9px;font-size:var(--fs-1)" onclick="openChit(\''+esc(r.chit_id)+'\')">' + tx('Open chit') + '</button>':'')+'</div>';
  }).join('');
  return head+'<div style="font-size:var(--fs-1);color:var(--grey);margin:4px 0 2px">Hash + outcome only — the raw payload is never stored.</div>'+rows;
}
...
function piCockpit(x){
  ...
  var ico=function(t,title,on){ return '<button title="'+title+'" onclick="'+on+'" style="border:1px solid var(--line);background:var(--card);border-radius:9px;width:34px;height:34px;cursor:pointer;font-size:15px;color:var(--on-card)">'+t+'</button>'; };
  var header='<div style="position:sticky;top:0;background:var(--card);z-index:5;padding-bottom:10px;border-bottom:1px solid var(--line);color:var(--on-card)">'
    ...
    +'<div style="font-size:var(--fs-1);color:var(--grey);margin-top:3px">📍 '+esc(x.site||'no site')+' · '+esc(health)+'</div></div>';
  var offline = health==='offline' ? '<div style="background:var(--danger-tint);border:1px solid #f0c9c6;color:var(--disp);border-radius:9px;padding:9px 11px;font-size:var(--fs-2);font-weight:600;margin:10px 0">⚠ '+(iot?'Gateway':'System')+' OFFLINE — no '+(iot?'device':'endpoint')+' below can signal until it is back.</div>' : '';
  ...
  var tiles='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin:14px 0">'
    ...
    +_tile('Live now', _loadingT?'—':('<span style="display:inline-block;width:9px;height:9px;border-radius:50%;vertical-align:middle;background:'+(_live>0?'var(--ok-3)':'var(--disp)')+';color:' + (_live>0 ? 'var(--on-ok)' : 'var(--on-danger)') + ';margin-inline-end:6px"></span>'+_live+' <small style="font-size:var(--fs-2);color:var(--grey-2);font-weight:600">/ '+_cn.length+'</small>'))
    +_tile('Last signal', _loadingT?'—':(_agoT||'—'))
    +'</div>';
  var conns=UI.acConns, list;
  if(conns===undefined) list='<div style="padding:16px;color:var(--grey);font-size:var(--fs-2)">' + tx('Loading…') + '</div>';
  else if(UI.acConnsErr) list='<div style="padding:10px 2px;color:var(--disp);font-size:var(--fs-2)">⚠ '+esc(UI.acConnsErr)+' <button class="composebtn" style="padding:2px 9px;font-size:var(--fs-1);margin-inline-start:6px" onclick="acLoadDevices(\''+x.id+'\')">' + tx('Retry') + '</button></div>';
  ...
  var installerBtn = iot ? '<button class="composebtn pri" style="width:100%;margin:8px 0 3px" onclick="acCreatePackage(\''+x.id+'\')">' + tx('📦 Get the Pi installer') + '</button><div style="font-size:var(--fs-1);color:var(--warn-2);text-align:center;margin-bottom:8px;line-height:1.45">' + txf('⚠ Downloading {reissues} — a device already running this gateway goes silent until you reflash it with the new installer.', { reissues: '<b>' + tx('reissues the key') + '</b>' }) + '</div>' : '';
  // ERP has no installer; its one-tap live-run is a sample document through the real process-then-forget cycle.
  var erpTestBtn = !iot ? '<button class="composebtn pri" style="width:100%;margin:8px 0 3px" onclick="acErpTest(\''+x.id+'\')">' + tx('📄 Send a test document') + '</button><div style="font-size:var(--fs-1);color:var(--grey);text-align:center;margin-bottom:8px;line-height:1.45">' + txf('Runs the whole loop once with a sample document.', {}) + specNote('connector.process-then-forget', 'The real <b>process-then-forget</b> cycle: a receipt (hash + outcome) is kept and a co-held chit is sent. The raw payload is never stored.') + '</div>' : '';
  var govLink = iot ? '<button class="composebtn" style="width:100%;margin:2px 0 9px;display:flex;align-items:center;justify-content:center;gap:7px;font-size:var(--fs-2)" onclick="if(typeof openBlueprint===\'function\')openBlueprint(\'iot-signal\')">🔒 What governs this signal — licence · sealed · open <span class=arw>→</span></button>' : '';
...
function _provPanel(iot, aid){
  var p=UI.acProv; if(!p) return '<div style="padding:10px 2px;color:var(--grey);font-size:var(--fs-2)">' + tx('Loading connection string…') + '</div>';
  var lines = iot ? 'Endpoint = '+esc(p.endpoint||'')+'\nActorKey = ••••••••  (secret — Regenerate to issue a fresh one)\nBridgeIds:\n'+((p.publishes||[]).map(function(z){return '  '+esc(z.user_id||z.bridge_id||'—')+'  '+esc(z.ref||'');}).join('\n')||'  (none yet)') : 'Base URL = '+esc(p.base_url||'—')+'\nAuth = '+esc(p.auth_type||'none')+(p.auth_ref?(' (ref: '+esc(p.auth_ref)+')'):'');
  var fresh = (iot && UI.acFreshKey) ? '<div style="border:1px solid #bfe6c9;background:var(--ok-tint);border-radius:9px;padding:9px 11px;margin-top:9px;color:var(--on-card)"><div style="font-size:var(--fs-1);font-weight:700;color:var(--ok-2);margin-bottom:5px">✅ New ActorKey — copy it to the Pi NOW, it will not be shown again:</div><pre style="background:var(--chrome);color:var(--blue-tint-line);border-radius:6px;padding:9px 11px;font-size:var(--fs-2);overflow:auto;margin:0;user-select:all">'+esc(UI.acFreshKey)+'</pre></div>' : '';
  var regen = (iot && aid) ? '<div style="margin-top:9px"><button class="composebtn" onclick="acRegenKey(\''+esc(aid)+'\')">' + tx('♻ Reissue raw key (advanced)') + '</button><div style="font-size:var(--fs-1);color:var(--grey);margin-top:4px">' + txf('For a {dev} that cannot run the installer — shows the raw key once. For a Pi, use {btn} above instead. Either way, reissuing stops any device on the old key until it is reflashed.', {
        dev: '<b>' + tx('direct HTTPS device') + ' (ESP32 etc.)</b>', btn: '<b>' + tx('📦 Get the Pi installer') + '</b>' }) + '</div></div>' : '';
  return '<div style="border:1px solid var(--blue-tint-line);background:var(--blue-tint-bg);border-radius:9px;padding:10px 12px;margin:10px 0;color:var(--on-card)"><div style="font-weight:700;font-size:var(--fs-2);color:var(--blue-2);margin-bottom:4px">' + tx('🔑 Connection string') + '</div><pre style="background:var(--chrome);color:var(--blue-tint-line);border-radius:6px;padding:9px 11px;font-size:var(--fs-1);overflow:auto;margin:0;line-height:1.5">'+lines+'</pre>'+fresh+regen+'</div>';
}
...
function _addDeviceForm(x,iot){
  ...
  var spec = iot ? '<label class="fl">' + tx('Topic') + '</label><input class="inp" id="ad_topic" data-testid="conn-topic" placeholder="sensors/line1/temp" style="width:100%"><label class="fl">' + tx('Device id') + '</label><input class="inp" id="ad_dev" data-testid="conn-device-id" placeholder="edge-gw-01" style="width:100%">'
                 : '<label class="fl">' + tx('Resource path') + '</label><input class="inp" id="ad_path" data-testid="conn-path" placeholder="/odata/PurchaseOrders" style="width:100%">';
  return '<div style="border:1px dashed #c9d2dd;border-radius:9px;padding:12px;margin:10px 0;background:var(--blue-tint-bg);color:var(--on-card)"><div style="font-weight:700;font-size:var(--fs-2);margin-bottom:2px">Add '+(iot?'device':'endpoint')+'</div>'
    +'<label class="fl">' + tx('Name') + '</label><input class="inp" id="ad_ref" data-testid="conn-device-name" placeholder="'+(iot?'Cold-store temp':'PO inbound')+'" style="width:100%">'+spec
    ...
    +'<div style="display:flex;gap:8px;margin-top:10px"><button class="composebtn pri" onclick="acAddDevice(\''+x.id+'\','+(iot?'true':'false')+')">' + tx('Add') + '</button><button class="composebtn" onclick="UI.acAddDev=false;paintAcDetail()">' + tx('Cancel') + '</button></div></div>';
}
...
function acReissueGate(id, mode){ UI.acReissue={id:id, mode:mode||'package'};
  ...
  modal('<div style="padding:2px 2px"><div style="font-weight:800;font-size:var(--fs-4);margin-bottom:4px">' + tx('⚠ Reissue device key') + '</div>'
    +'<div style="font-size:var(--fs-2);color:var(--grey-2);line-height:1.5;margin-bottom:12px">' + txf('This issues a {newkey} — any device already running this gateway {stops} until you reflash it with the new installer. Only the owner or a manager may do this.', { newkey: '<b>' + tx('new key') + '</b>', stops: '<b>' + tx('stops') + '</b>' }) + '</div>'
    ...
    +'<label class="fl">' + tx('2 · One-time code (sent to the account email)') + '</label><div style="display:flex;gap:8px;align-items:center;margin-bottom:4px"><input class="inp" id="rg_otp" inputmode="numeric" maxlength="6" placeholder="6-digit code" style="flex:1"><button class="composebtn" onclick="acReissueSendCode()">' + tx('Send code') + '</button></div>'
    +'<div id="rg_msg" style="font-size:var(--fs-1);color:var(--blue-2);min-height:15px;margin-bottom:10px"></div>'
    +'<div style="display:flex;gap:10px"><button class="composebtn" style="flex:1" onclick="closeModal()">' + tx('Cancel') + '</button><button class="composebtn pri" style="flex:1" onclick="acReissueSubmit()">' + tx('Reissue') + '</button></div>'
    +'</div>', false);
}
