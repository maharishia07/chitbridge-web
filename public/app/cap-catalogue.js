...
function _cwStep4(w){
  ...
  if (mode === 'csv') {
    ...
    body = '<div style="font-size:var(--fs-2);color:var(--ink-2);margin-bottom:8px">Only have <b>photos or product labels</b>? Add the pictures — each becomes an item. You confirm the name &amp; price (the same human-confirm step the <b>' + tx('Capture') + '</b> connector uses).</div>'
      ...
      + (w._phErr ? '<div style="margin-top:8px;background:var(--danger-tint);border:1px solid #f0c9c6;border-radius:9px;padding:8px 11px;font-size:12px;color:var(--disp)">' + esc(w._phErr) + '</div>' : '')
      + (w._phNote ? '<div style="margin-top:8px;background:var(--blue-tint-bg);border:1px solid #e4dff6;border-radius:9px;padding:8px 11px;font-size:12px;color:var(--blue)">' + esc(w._phNote) + '</div>' : '')
...
function _catfItemsHtml(f){
  ...
  var rows = items.map(function(it, i){
    ...
    var priceCell = (it.price != null && it.price !== '')
      ...
      : '<input type="number" placeholder="set price" onchange="catfSetItemPrice(' + i + ',this.value)" title="the ONLY thing you enter for a referenced item" style="width:92px;padding:4px 7px;border:1px solid #d98b84;border-radius:6px;font-size:12px;background:var(--danger-tint);color:var(--on-card)">';
