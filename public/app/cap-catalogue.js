...
function _cwProductCard(w, it){
  ...
  return '<div onclick="cwSelectProduct(\'' + nm + '\')" style="border:1px solid ' + (seld ? accent : 'var(--line)') + ';border-radius:12px;padding:10px 12px;margin-top:6px;background:' + (seld ? accent + '0c' : 'var(--card)') + ';cursor:pointer">'
    + (it.photo ? '<div style="height:96px;background:var(--card) center/cover no-repeat;background-image:url(' + it.photo + ');border-radius:9px;margin-bottom:7px"></div>' : '')
    ...
    body = '<div style="font-size:var(--fs-2);color:var(--ink-2);margin-bottom:8px">Only have <b>photos or product labels</b>? Add the pictures — each becomes an item. You confirm the name &amp; price (the same human-confirm step the <b>Capture</b> connector uses).</div>'
      ...
      + (ph.length ? '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(148px,1fr));gap:10px;margin-top:12px">'
          + ph.map(function(p){ return '<div style="border:1px solid var(--line);border-radius:9px;padding:8px;background:var(--card)">'
            + '<div style="height:88px;background:var(--card) center/cover no-repeat;background-image:url(' + p.url + ');border-radius:6px"></div>'
            /**
...
function _catfItemsHtml(f){
  ...
  var rows = items.map(function(it, i){
    ...
    var priceCell = (it.price != null && it.price !== '')
      ...
      : '<input type="number" placeholder="set price" onchange="catfSetItemPrice(' + i + ',this.value)" title="the ONLY thing you enter for a referenced item" style="width:92px;padding:4px 7px;border:1px solid #d98b84;border-radius:6px;font-size:12px;background:var(--danger-tint)">';
    return '<div style="border:1px solid var(--line);border-radius:9px;padding:8px 11px;margin-top:5px;background:var(--card)"><div style="display:flex;align-items:center;gap:8px">'
      + ((it._photo || it.photo) ? '<span style="width:26px;height:26px;border-radius:5px;background:var(--card) center/cover no-repeat;background-image:url(' + (it._photo || it.photo) + ');flex:none"></span>' : (it._media ? '<span style="width:22px;height:22px;border-radius:5px;background:linear-gradient(135deg,#eef1f5,#dde3ea);flex:none"></span>' : ''))
