    ...
    body = '<div style="font-size:var(--fs-2);color:var(--ink-2);margin-bottom:8px">' + txf('Only have {photos}? Add the pictures — each becomes an item, and you confirm the name and price.', { photos: '<b>' + tx('photos or product labels') + '</b>' }) + specNote('catalogue.photos.confirm', 'The same human-confirm step the <b>Capture</b> connector uses.') + '</div>'
      ...
      + (ph.length ? '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(148px,1fr));gap:10px;margin-top:12px">'
          + ph.map(function(p){ return '<div style="border:1px solid var(--line);border-radius:9px;padding:8px;background:var(--card);color:var(--on-card)">'
            ...
            + '<div style="display:flex;gap:5px;margin-top:5px;align-items:center"><span style="font-size:var(--fs-1);color:var(--grey)">' + esc(_catfCcy()) + '</span><input type="number" value="' + (p.price != null ? p.price : '') + '" oninput="cwSetPhotoField(\'' + p.id + '\',\'price\',this.value)" placeholder="price" style="flex:1;min-width:0;padding:4px 6px;border:1px solid var(--line);border-radius:6px;font-size:var(--fs-1)"><span onclick="cwPhotoRemove(\'' + p.id + '\')" style="cursor:pointer;color:#b23;font-size:15px;line-height:1" title="remove">×</span></div>'
            + '</div>'; }).join('') + '</div>'
