...
const CAP_CATALOGUE = [
  ...
  { id:'connector', name:'Connector · IoT / ERP', icon:'🛰️', load:'lazy', maturity:1, target:2,
    blurb:'Non-human actors — a Pi (IoT) or a system (ERP) — that exchange chits over the rail, now managed INSIDE Co-assists (one panel, same shell as people). A Pi holds its connection string; its devices/endpoints are connections under it. b62 = connection-string schema + provisioning + cascading health. Built & deployed 2026-07-06; L1→L2 gate = one real signal live-run (not yet human-run). L5 = governed fleets, billed.',
    features:[
      {n:'Connector = actor grouped by SITE; connection string on the actor (b62)', s:'done'},
      {n:'Devices/endpoints per Pi (own bridge_id + config); enable/disable; add inline', s:'done'},
      {n:'Provisioning — CB-issued ActorKey (shown once), push/pull; ERP base-url + auth-ref', s:'done'},
      ...
      {n:'Live device / sim actually ingesting → signal becomes a chit (L1→L2 live-run gate)', s:'backlog'},
