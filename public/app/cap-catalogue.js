...
function _cwStep2Products(w){
  ...
  var products = (w.built && w.built.finishes) ? '<div style="margin-top:12px;display:flex;align-items:center;gap:8px"><span style="font-size:11px;font-weight:800;color:#6a44a8;letter-spacing:.05em">PRODUCTS · ' + w.built.finishes.length + '</span><span style="font-size:9.5px;color:var(--grey)">tick to sell · click to inspect</span><span onclick="cwChooseAll(true)" style="cursor:pointer;font-size:10px;color:var(--blue);margin-left:auto">all</span><span style="color:#c8d0d9">·</span><span onclick="cwChooseAll(false)" style="cursor:pointer;font-size:10px;color:var(--blue)">none</span></div>' + w.built.finishes.map(function(it){ var on = w.chosen[it.name] !== false; var seld = w.sel === it.name; var nm = esc(it.name).replace(/'/g, "\\'"); return '<div onclick="cwSelectProduct(\'' + nm + '\')" style="display:flex;align-items:center;gap:9px;padding:6px 10px;border:1px solid ' + (seld ? '#2c5aa0' : 'var(--line)') + ';border-radius:9px;margin-top:5px;background:' + (seld ? '#eef4fc' : '#fff') + ';cursor:pointer"><input type="checkbox" ' + (on ? 'checked' : '') + ' onclick="event.stopPropagation()" onchange="cwToggleItem(\'' + nm + '\')"><span style="font-weight:600;font-size:12.5px;flex:1">' + esc(it.name) + '</span><span style="font-size:10px;color:var(--grey)">' + esc(it.texture_family || '') + '</span><span style="color:#2c5aa0;font-size:11px">' + (seld ? '▸' : '') + '</span></div>'; }).join('') : '';
