...
function _provPanel(iot, aid){
  ...
  var fresh = (iot && UI.acFreshKey) ? '<div style="border:1px solid #bfe6c9;background:var(--ok-tint);border-radius:9px;padding:9px 11px;margin-top:9px"><div style="font-size:11.5px;font-weight:700;color:var(--ok-2);margin-bottom:5px">✅ New ActorKey — copy it to the Pi NOW, it will not be shown again:</div><pre style="background:var(--chrome);color:var(--blue-tint-line);border-radius:6px;padding:9px 11px;font-size:12px;overflow:auto;margin:0;user-select:all">'+esc(UI.acFreshKey)+'</pre></div>' : '';
...
function _addDeviceForm(x,iot){
  ...
  return '<div style="border:1px dashed #c9d2dd;border-radius:9px;padding:12px;margin:10px 0;background:var(--blue-tint-bg)"><div style="font-weight:700;font-size:var(--fs-2);margin-bottom:2px">Add '+(iot?'device':'endpoint')+'</div>'
