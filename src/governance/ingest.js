// Source = input (raw regulation) -> process (AI interpretation) -> output (typed pack + MoSCoW + context).
// In production the process is a GOVERNED AI call. Here it is a canned interpretation so the simulator
// stays deterministic — and the screen openly shows the AI runs only at the EDGE. The attest gate is REAL.

const REGULATIONS = {
  gdp:   { id:'gdp',   label:'GDP — Good Distribution Practice (pharma)',
           raw:'Distributors shall maintain temperature records throughout the cold chain, ensure batch traceability, and enable recall of any batch.' },
  gdpr:  { id:'gdpr',  label:'GDPR Art.5 — minimisation & residency',
           raw:'Personal data shall be processed lawfully, kept no longer than necessary, and not transferred outside the permitted region.' },
  fssai: { id:'fssai', label:'FSSAI — food safety (India)',
           raw:'Food businesses shall record critical control points, maintain lot traceability, and display licence on every consignment.' },
};
...
// process step (simulated AI interpretation)
...
// the attest gate — REAL. Must / Class-A items require human sign-off before register.
export const attest = (pack, attestor) => ({ ...pack, attested:true, attestor: attestor || 'domain expert' });

// register -> a source pack the cascade/data-definition could consume
